...
namespace LogLineHandler
{
   ...
   public class WFSCIMCASHINFO : WFSCUINFO
   {
      ...
      protected override void Initialize()
      {
         ...
         // There are 3 formats of CIM_CASH_UNIT_INFO: 
         // 1. Table Form (contains "lppCashIn->") and is a complete dump of all data
         // 2. List Form (contains "lppCashIn =") and contains multiple units in a list format
         // 3. List Form with a single unit (does not contain lppCashIn), really a degenerate case of List Form
         ...
         try
         {
            ...
            else if (indexOfNull == -1 && logLine.Contains("usNumber = ["))
            {
               // L O G I C A L

               // In List Form, if the list contains usCount there are mulitple units, otherwise there is just one.
               ...
               // In List Form, if the log is truncated we have incomplete data so reduce the number of LCU by 1
         ...
         {
            Console.WriteLine(String.Format("Failed to parse WFSCIMCASHINFO log line in WFSCIMCASHINFO {0}.", logLine.Substring(0, 200)));
         }
      }
   }
}
