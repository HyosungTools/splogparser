...
namespace splogparser
{
   class Program
   {
      ...
      private static void Run(Options opts)
      {
         ...
         ctx.ConsoleWriteLogLine("opts.MetaViews :" + ctx.opts.MetaViews);
         ...
         ctx.ConsoleWriteLogLine(String.Format("IsMeta : {0}", ctx.opts.IsMeta ? "true" : "false"));
         ctx.ConsoleWriteLogLine(String.Format("MetaView Contains  : {0}", ctx.opts.MetaViews));
         ...
         // Meta
         if (ctx.opts.IsMeta) ctx.logFileHandlers.Add((ILogFileHandler)new IILogHandler(new CreateTextStreamReader()));
         ...
         // NOTE: ctx.combinedDataset now contains all the tables generated by the views, available for the Analyze steps
